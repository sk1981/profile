...
export default {
  handleKeyMove(
    newHighlightedOptionIndex: number,
    filteredOptions: any,
    handler: any
  ) {
    if (
      newHighlightedOptionIndex < filteredOptions.length &&
      newHighlightedOptionIndex > -2
    ) {
  ...
  render(model: any, handler: any) {
    return h("div.input.select-type__wrapper", {}, [
      h(
        "input.select-type__input",
        {
          class: {
            "select-type__input--selected":
              !model.active && !!model.selectedText
          },
          attrs: {
