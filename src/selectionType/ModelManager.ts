export default {
  ...
  filterOptions: function(model: any, newText: string) {
    ...
    model.filteredOptions = model.options.filter(
      (option: string) => option.toLowerCase().indexOf(lowerCaseText) > -1
    );
    //As filtered options have changed we need to ensure highlighted options are within range
    model.highlightedOptionIndex = Math.min(
      model.highlightedOptionIndex,
      model.filteredOptions.length - 1
    );
    return model;
  }
};
